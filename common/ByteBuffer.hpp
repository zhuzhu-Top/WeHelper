...
public:
    ...
    void __inline reserve(uint32_t newSize) {
        buf.reserve(newSize);
    }
    //收缩大小
    void __inline shrink(uint32_t newSize) {
        buf.resize(newSize);
    }
	...
	void replace(uint8_t* data, uint8_t size, uint32_t start = 0);
    ...
    void putHexString(const std::string_view& sv);
    void putHexString(const std::string& s) { return putHexString(std::string_view(s)); }
    ...
    void remove(uint32_t startIndex,uint32_t length){
        buf.erase(buf.begin()+startIndex,buf.begin()+startIndex+length);
        wpos=0;
        rpos=0;
    }
private:
	...
	std::string name;
#endif
    friend std::ostream& operator<<(std::ostream& ostream, const ByteBuffer& arr);
